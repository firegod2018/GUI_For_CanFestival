   ...
   #[ID_HTMLFRAME, ID_HTMLFRAMEHTMLCONTENT] = [wx.NewId() for _init_ctrls in range(2)]
    ...
    class HtmlFrame(wx.Frame):
        def _init_ctrls(self, prnt):
            wx.Frame.__init__(self, id=wx.ID_ANY, name='HtmlFrame',
                  parent=prnt, pos=wx.Point(320, 231), size=wx.Size(853, 616),
                  style=wx.DEFAULT_FRAME_STYLE, title='')
            self.Bind(wx.EVT_CLOSE, self.OnCloseFrame)
            
            #self.HtmlContent = UrlClickHtmlWindow(id=wx.ID_ANY,
            ...
            self.HtmlContent = wx.html.HtmlWindow(id=wx.ID_ANY,
                  name='HtmlContent', parent=self, pos=wx.Point(0, 0),
                  size=wx.Size(-1, -1), style=wx.html.HW_SCROLLBAR_AUTO|wx.html.HW_NO_SELECTION)
...
class objdictedit(wx.Frame, NodeEditorTemplate):
    ...
    def _init_coll_FileMenu_Items(self, parent):
        ...
        parent.AppendSeparator()
        parent.Append(helpString='', id=wx.ID_ANY,
        ...
        self.Bind(wx.EVT_MENU, self.OnImportEDSMenu,
              id=parent.FindItem(_("Import EDS file")))
        self.Bind(wx.EVT_MENU, self.OnExportEDSMenu,
              id=parent.FindItem(_("Export to EDS file")))
        self.Bind(wx.EVT_MENU, self.OnExportCMenu,
              id=parent.FindItem(_("Build Dictionary\tCTRL+B")))
    ...
    def _init_coll_EditMenu_Items(self, parent):
        ...
        self.Bind(wx.EVT_MENU, self.OnNodeInfosMenu,
              id=parent.FindItem(_("Node infos")))
        self.Bind(wx.EVT_MENU, self.OnCommunicationMenu,
              id=parent.FindItem(_("DS-301 Profile")))
        self.Bind(wx.EVT_MENU, self.OnOtherCommunicationMenu,
              id=parent.FindItem(_("DS-302 Profile")))
        self.Bind(wx.EVT_MENU, self.OnEditProfileMenu,
              id=parent.FindItem(_("Other Profile")))
        self.EDITMENU_ID = parent.FindItem(_("Other Profile"))
    ...
    def _init_ctrls(self, prnt):
        ...
        self.HelpBar = wx.StatusBar(id=wx.ID_ANY, name='HelpBar',
    ...
    def __init__(self, parent, manager = None, filesOpen = []):
        ...
        if self.Manager.CurrentDS302Defined():
            self.EditMenu.Enable(self.EditMenu.FindItem(_("DS-302 Profile")), True)
        else:
            self.EditMenu.Enable(self.EditMenu.FindItem(_("DS-302 Profile")), False)
    ...
    def RefreshMainMenu(self):
        if self.FileOpened.GetPageCount() > 0:
            if self.ModeSolo:
                ...
                self.FileMenu.Enable(self.FileMenu.FindItem(_("Export to EDS file")), True)
                self.FileMenu.Enable(self.FileMenu.FindItem(_("Build Dictionary\tCTRL+B")), True)
            else:
            ...
            if self.ModeSolo:
                ...
                self.FileMenu.Enable(self.FileMenu.FindItem(_("Export to EDS file")), False)
                self.FileMenu.Enable(self.FileMenu.FindItem(_("Build Dictionary\tCTRL+B")), False)
            else:
...
def format_namespace(d, indent='    '):
    return '\n'.join(['%s%s: %s' % (indent, k, repr(v)[:10000]) for k, v in list(d.items())])
