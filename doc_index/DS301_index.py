...
def get_acroversion():
    import winreg
    appPathReg = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE,r"Software\Microsoft\Windows\CurrentVersion\App Paths")
    for index in range(winreg.QueryInfoKey(appPathReg)[0]):
        key = winreg.EnumKey(appPathReg,index)
        result = None
        if "foxitreader" in key.lower() or "acrobat" in key.lower():
            result = winreg.QueryValue(winreg.HKEY_LOCAL_MACHINE,"Software\Microsoft\Windows\CurrentVersion\App Paths\%s"%key)
            return result
