                ...
                if mappingdictionary[index]["values"][subIndex]["pdo"]:
                    ...
                    if mappingdictionary[index]["struct"] & OD_IdenticalSubindexes:
                        ...
                        for i in range(len(values) - 1):
                            ...
                            ListRet.append((index, i + 1, infos["size"], computed_name))
                    else:
                        computed_name = name
                        if compute:
                            computed_name = StringFormat(computed_name, 1, subIndex)
                        ListRet.append((index, subIndex, infos["size"], computed_name))
    return ListRet
...
#-------------------------------------------------------------------------------
    ...
    def GetMapVariableList(self, compute=True):
        ListRet = FindMapVariableList(MappingDictionary, self, compute)
        for mapping in self.GetMappings():
            ListRet.extend(FindMapVariableList(mapping, self, compute))
        ListRet.sort()
        return ListRet
